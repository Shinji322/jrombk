...
void blowBomb(Bomb *bomb) {
  ...
  for (int y = 0; y < MAP_HEIGHT; y++) {
    for (int x = 0; x < MAP_WIDTH; x++) {
      float distPlayer1 =
          sqrt(pow((player1.x - bomb->x) / 2, 2) + pow(player1.y - bomb->y, 2));
      float distPlayer2 =
          sqrt(pow((player2.x - bomb->x) / 2, 2) + pow(player2.y - bomb->y, 2));
...
void removeBomb(Bomb *bomb) {
  ...
  for (int y = 0; y < MAP_HEIGHT; y++) {
    for (int x = 0; x < MAP_WIDTH; x++) {
      double currentDistanceFromCenter =
          sqrt(pow((x - bomb->x) / 2, 2) + pow(y - bomb->y, 2));
